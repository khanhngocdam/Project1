...
int hash_index(char word[]) {
  return ((word[0] - 97) % 26) * 10 + (word[1] - 97) % 26;
}
// Viet vao file 
void writeHashToFile(sllist_w** hash_list, ofstream *f) {
  for(int i = 0; i < 275; i++) {
    for(slnode_w* p_w = hash_list[i] ->front; p_w!= NULL; p_w = p_w -> next) {
      *f << setw(10) << left << p_w -> word;
      *f << setw(5) << left << p_w -> count << i << " Line:   ";
